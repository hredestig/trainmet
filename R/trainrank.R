...
trainrank <- function(mdat, tdat, states=3, cyclic=FALSE,
                      ...
                      trainrank=system.file("extdata", "train_rank.py",
                        package="trainmet"),
  ...
  if(length(grep(LD_LIBRARY_PATH, Sys.getenv("LD_LIBRARY_PATH"))) == 0)
    Sys.setenv(LD_LIBRARY_PATH=paste("/usr/local/lib/",
  ...
  return(list(positive=pos,negative=neg,plag=poslag,nlag=neglag))
}
